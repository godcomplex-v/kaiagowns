...
$url_pattern = APP_URL . '/customer/notifications.php?' . ($filter?'filter='.e($filter).'&':'') . 'page=%d';
...
<main class="main-content">
    <header class="topbar"><h2>Notifications</h2></header>
    <section class="page-body">
        ...
        <div class="notif-full-list">
            ...
            <?php else: ?>
                <?php
                $icons = [
                    'approved'=>'✅','rejected'=>'❌','return_received'=>'📦',
                    'completed'=>'🎉','overdue'=>'⚠','info'=>'ℹ'
                ];
                foreach ($notifs as $n):
                ?>
                <div class="notif-full-item <?= !$n['is_read']?'notif-full-item--unread':'' ?>"
                     id="notif-<?= (int)$n['id'] ?>">
                    <div class="notif-full-icon"><?= $icons[$n['type']] ?? 'ℹ' ?></div>
                    <div class="notif-full-body">
                        <p class="notif-full-msg"><?= e($n['message']) ?></p>
                        <span class="notif-full-time"><?= fmt_date($n['created_at'],'M d, Y H:i') ?></span>
                    </div>
                    <?php if (!$n['is_read']): ?>
